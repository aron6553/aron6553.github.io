		...
		if($fileError === 0){
			if($fileSize < 1000000){
				...
				if(empty($imageTitle) || empty($imageDesc)){
					header("location: ../Aryenn.php?upload=empty");
					exit();
				}else{
					//prepared statements
					$sql = "SELECT * FROM gallery1;";
					...
					if(!mysqli_stmt_prepare($stmt, $sql)){
						...
						$setImageOrder = $rowCount + 1;
							...
							$sql = "INSERT INTO gallery1 (titleGallery, descGallery, imgFullNameGallery, orderGallery) VALUES(?, ?, ?, ?);";
							if(!mysqli_stmt_prepare($stmt, $sql)){
						echo "SQL error!";
					}else{
