	...
	if(in_array($fileActualExt, $allowed)){
		if($fileError === 0){
			if($fileSize < 2000000){
				...
				$fileDestination = '../uploads/Gallerygarage/'.$fileNameNew;
				move_uploaded_file($fileTempName, $fileDestination);
				$sql = "UPDATE imguploadgarage SET status=0 WHERE userid='$username';";
				$result = mysqli_query($connn, $sql);
				header("Location: ../garagepage.php?upload=success");
			}else{
